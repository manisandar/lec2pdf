...
MIN_SLIDE_AREA_FRACTION: float = 0.30
...
def _four_point_transform(image, pts) -> "np.ndarray":
    ...
    return warped
def _trim_black_border(img_pil):
    import cv2
    import numpy as np
    ...
    mask = np.where(L > 185, 255, 0).astype("uint8")
...
def _find_slide_contour(
    ...
    for thresh_val in BRIGHT_THRESHOLDS:
        _, bright_mask = cv2.threshold(work_gray,thresh_val,255,cv2.THRESH_BINARY)

        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (15, 15))
        bright_mask = cv2.morphologyEx(bright_mask,cv2.MORPH_CLOSE,kernel)
        bright_mask = cv2.morphologyEx(bright_mask,cv2.MORPH_OPEN,kernel)

        contours, _ = cv2.findContours(bright_mask,cv2.RETR_LIST,cv2.CHAIN_APPROX_SIMPLE)
        ...
        for c in contours:
            ...
            area_ratio = area / (ww * wh)

        # Reject huge objects like the ceiling
            if area_ratio > 0.65:
                continue
        ...
        # Slides are landscape
            if aspect < 1.1:
                continue

        # Ignore contours touching image borders
            margin = 20
...
def _process_single_rotation(img_pil) -> "PIL.Image.Image":
    ...
    if target is not None:
        ...
        warped = Image.fromarray(cv2.cvtColor(warped, cv2.COLOR_BGR2RGB))
        warped = _trim_black_border(warped)
        return warped
